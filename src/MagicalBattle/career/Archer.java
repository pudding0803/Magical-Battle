...
public class Archer extends Player {
    ...
    public void attack() {
        ...
        int arrowsNumber = new Random().nextInt(7) + 6;
        for (int i = 0; i < arrowsNumber; i++) {
            int imageIndex = (new Random().nextInt(4) == 0 ? 1 : 0);
            int fixedPositionX = new Random().nextInt(41) - 20;
            int fixedPositionY = new Random().nextInt(61) - 30;
            int fixedVelocityX = new Random().nextInt(8) - 2;
            ...
            SkillObject skillObject = new SkillObject(attack, statusList, damage, this.isPlayer1, (index != -1 ? -Settings.ATTACK_VELOCITY : Settings.ATTACK_VELOCITY) + fixedVelocityX, 0);

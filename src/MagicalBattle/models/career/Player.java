...
import MagicalBattle.models.effectObject.DizzyEffect;
import MagicalBattle.models.effectObject.MissLabel;
import MagicalBattle.models.effectObject.StunnedEffect;
...
public abstract class Player {
    private static final String assetsFilePath = "../../assets/";
    ...
    public void setVelocity() {
        if (this.isUp() && this.jumpCount-- > 0) {
            AudioClip audioClip = new AudioClip(Objects.requireNonNull(getClass().getResource(assetsFilePath + "media/other/jump.mp3")).toExternalForm());
    ...
    public void doHorizonMotion() {
        ...
        if (this.isLeft()) {
            ...
            else this.self.setImage(imageSetMap.get(this.career).getLeft(index == Settings.MOVE_IMAGES_COUNT - 1 ? 0 : ++index));
            ...
            else this.self.setImage(imageSetMap.get(this.career).getRight(index == Settings.MOVE_IMAGES_COUNT - 1 ? 0 : ++index));
        }
        ...
        KeyFrame keyFrame = new KeyFrame(Duration.millis(Settings.UPDATE_TIME), (event) -> {
            ...
            if (this.getY() == Settings.GROUND_HEIGHT - this.getHeight()) {
                if (this.jumpCount <= (this.career == Career.ASSASSIN ? 1 : 0)) {
                    AudioClip audioClip = new AudioClip(Objects.requireNonNull(getClass().getResource(assetsFilePath + "media/other/land.mp3")).toExternalForm());
                    audioClip.setVolume(Settings.EFFECT_VOLUME);
                    audioClip.play();
                }
    ...
    public boolean isCollidedFromOther(SkillObject skillObject) {
        ...
        if (skillObject.isFromOther(this.isPlayer1) && (inSelf(x, y) || inSelf(x + width, y) || inSelf(x, y + height) || inSelf(x + width, y + height))) {
            ...
            if (miss) {
                AudioClip audioClip = new AudioClip(Objects.requireNonNull(getClass().getResource(assetsFilePath + "media/other/miss.mp3")).toExternalForm());
                audioClip.setVolume(Settings.EFFECT_VOLUME);
                audioClip.play();
                ...
                if (skillObject.containStunned()) {
                    AudioClip audioClip = new AudioClip(Objects.requireNonNull(getClass().getResource(assetsFilePath + "media/effect/stunned.mp3")).toExternalForm());
                    audioClip.setVolume(Settings.EFFECT_VOLUME);
                    audioClip.play();
                ...
                if (skillObject.containDizzy()) {
                    AudioClip audioClip = new AudioClip(Objects.requireNonNull(getClass().getResource(assetsFilePath + "media/effect/dizzy.mp3")).toExternalForm());
                    audioClip.setVolume(Settings.EFFECT_VOLUME);
                    audioClip.play();
    ...
    public void updateEffect() {
        ...
        if (!this.timer.isBurnedTimerEnd()) {
            if(this.timer.getBurnedTimer() % 50 == 0) {
                AudioClip audioClip = new AudioClip(Objects.requireNonNull(getClass().getResource(assetsFilePath + "media/effect/burned.mp3")).toExternalForm());
                audioClip.setVolume(Settings.EFFECT_VOLUME);
                audioClip.play();
